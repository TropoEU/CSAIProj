...
export async function handleOAuthCallback(req, res) {
  try {
    ...
    if (error) {
      ...
      return res.redirect(
        `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/email-channels?error=${encodeURIComponent(error)}`
      );
    }

    if (!code) {
      return res.redirect(
        `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/email-channels?error=no_code`
      );
    }
    ...
    try {
      ...
      return res.redirect(
        `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/email-channels?error=invalid_state`
      );
    }
    ...
    if (clientId === 'platform') {
      ...
      return res.redirect(
        `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/settings?success=platform_email_connected`
      );
    }
    ...
    if (channel) {
      ...
      logger.log('[EmailController] Updated existing email channel', {
      ...
      await EmailChannel.updateStatus(channel.id, 'active');
      logger.log('[EmailController] Created new email channel', {
        channelId: channel.id,
        email: emailAddress,
      });
    }
    ...
    res.redirect(
      `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/clients/${clientId}?tab=email&success=connected`
    );
  } catch (error) {
    console.error('[EmailController] OAuth callback error:', error);
    res.redirect(
      `${process.env.ADMIN_DASHBOARD_URL || 'http://localhost:3002'}/email-channels?error=${encodeURIComponent(error.message)}`
    );
  }
}
...
export async function disconnectChannel(req, res) {
  try {
    ...
    logger.log('[EmailController] Email channel disconnected', {
      channelId: channel.id,
    ...
    res
      ...
      .json({ error: 'Failed to disconnect email channel' });
  }
}
...
export async function testConnection(req, res) {
  try {
    ...
    res
      .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
      .json({ error: 'Failed to test connection', details: error.message });
  }
}
...
export async function getUnreadEmails(req, res) {
  try {
    ...
    res
      .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
      .json({ error: 'Failed to get unread emails', details: error.message });
  }
}
