...
export class ClientIntegration {
  ...
  static async findById(id) {
    const result = await db.query('SELECT * FROM client_integrations WHERE id = $1', [id]);
  ...
  static async delete(id) {
    const result = await db.query('DELETE FROM client_integrations WHERE id = $1 RETURNING *', [
      id,
    ]);
    return result.rows[0];
  }
