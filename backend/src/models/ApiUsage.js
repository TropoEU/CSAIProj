...
export class ApiUsage {
     ...
     */
    static async recordUsage(clientId, tokensInput, tokensOutput, toolCallsCount = 0) {
        const date = new Date().toISOString().split('T')[0]; // YYYY-MM-DD

        // Calculate cost estimate (example pricing - adjust as needed)
        // GPT-4o: $2.50 per 1M input tokens, $10 per 1M output tokens
        const costEstimate = (tokensInput / 1000000 * 2.50) + (tokensOutput / 1000000 * 10);
        ...
        const result = await db.query(
            `INSERT INTO api_usage (client_id, date, conversation_count, message_count, tokens_input, tokens_output, tool_calls_count, cost_estimate)
             ...
             DO UPDATE SET
                conversation_count = api_usage.conversation_count,
                ...
                updated_at = NOW()
             RETURNING *`,
            [clientId, date, tokensInput, tokensOutput, toolCallsCount, costEstimate]
        );
