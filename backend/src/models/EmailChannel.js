...
export class EmailChannel {
  ...
  static async findById(id) {
    const result = await db.query('SELECT * FROM email_channels WHERE id = $1', [id]);
  ...
  static async update(id, updates) {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (allowedFields.includes(key)) {
        // Handle JSONB fields
        if (
          (key === 'connection_config' || key === 'settings') &&
          value !== null &&
          typeof value === 'object'
        ) {
  ...
  static async delete(id) {
    const result = await db.query('DELETE FROM email_channels WHERE id = $1 RETURNING *', [id]);
    return result.rows[0];
  }
  ...
  static async deleteByClient(clientId) {
    const result = await db.query('DELETE FROM email_channels WHERE client_id = $1 RETURNING *', [
      clientId,
    ]);
    return result.rows;
  }
