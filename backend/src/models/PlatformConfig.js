...
export class PlatformConfig {
  ...
  static async delete(key) {
    const result = await db.query('DELETE FROM platform_config WHERE key = $1 RETURNING key', [
      key,
    ]);
      ...
      reasoning_steps: [
        {
          title: 'UNDERSTAND',
          instruction:
            'What is the customer actually asking for? Is this a question, request, complaint, or action?',
        },
        {
          title: 'CHECK CONTEXT',
          instruction:
            'Review the conversation history and business information. If the answer is in context, do NOT call a tool.',
        },
        {
          title: 'DECIDE',
          instruction:
            'If you can answer from context, respond directly. If you need external data, use a tool. If missing required info, ask ONE clear question.',
        },
        {
          title: 'RESPOND',
          instruction:
            'Keep responses to 1-2 sentences. Be friendly but concise. Never show JSON or technical details.',
        },
      ],
      response_style: {
        tone: 'friendly',
  ...
  static getAdaptiveDefaults() {
    return {
      ...
      reasoning_steps: [
        {
          title: 'UNDERSTAND',
          instruction: 'What is the user asking? Is it a greeting, question, or action request?',
        },
        {
          title: 'DECIDE',
          instruction:
            'Can I answer directly? Tool needed? Only request context if REQUIRED to answer.',
        },
      ],
      context_keys: [
