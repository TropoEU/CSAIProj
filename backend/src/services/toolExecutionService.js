...
6. If the result indicates an error or issue, explain it helpfully`;
    ...
    async executeStandardToolCall(toolCall, client, conversation, messages) {
        ...
        if (!validation.valid) {
            ...
            await ToolExecution.create(conversation.id, name, toolArgs, { error: validation.errors }, false, 0, 'blocked', errorDetails);
            await Message.createDebug(conversation.id, 'assistant', errorMessage, 'tool_result', {
                toolCallId: id,
                metadata: { tool_name: name, success: false, status: 'blocked', error_reason: errorDetails }
            });
        ...
        try {
            ...
            // Log execution
            await ToolExecution.create(conversation.id, name, finalArgs, result.data, result.success, result.executionTimeMs);
        } finally {
        ...
        await Message.createDebug(conversation.id, 'assistant', result.success ? formattedResult : `Error: ${result.error}`, 'tool_result', {
            toolCallId: id,
            metadata: { tool_name: name, success: result.success, execution_time_ms: result.executionTimeMs, raw_result: truncatedResult }
        });
