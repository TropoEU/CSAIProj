...
class AdaptiveReasoningService {
  ...
  async processAdaptiveMessage(conversationId, clientId, userMessage, client, conversation) {
    ...
    try {
      ...
      const recentMessages = await Message.getRecent(
        conversationId,
        ADAPTIVE_REASONING.CONTEXT_MESSAGE_COUNT
      );
      const formattedHistory = formatConversationHistory(recentMessages);

      // Store system prompt on first message
      if (recentMessages.length <= 1) {
        await Message.createDebug(conversationId, 'system', systemPrompt, 'system', {
      ...
      const { visible_response, assessment, reasoning } = llmService.parseAssessment(
        llmResponse.content
      );
      ...
      const needsCritique =
        ...
        this.shouldTriggerCritique(finalAssessment, tools);
      ...
      const critiqueResult = await this.runCritique(
        userMessage,
        finalAssessment,
        tools,
        formattedHistory,
      ...
      await Message.createDebug(conversationId, 'assistant', critiqueDisplay, 'critique', {
        metadata: critiqueDebugMetadata,
        reasonCode: REASON_CODES.CRITIQUE_TRIGGERED,
      });
  ...
  async _handleContextFetching(
    ...
    while (
      ...
      contextFetchCount++;
      console.log(
        `[AdaptiveReasoning] Context fetch attempt ${contextFetchCount}/${ADAPTIVE_REASONING.MAX_CONTEXT_FETCHES}:`,
        currentAssessment.needs_more_context
      );

      const { success, context, missing } = fetchContext(
        client,
        currentAssessment.needs_more_context
      );
      ...
      const { visible_response: newResponse, assessment: newAssessment } =
        llmService.parseAssessment(contextResponse.content);
      ...
      await Message.createDebug(
        ...
        {
          metadata: {
            context_keys: currentAssessment.needs_more_context,
            fetched: Object.keys(context),
            missing,
          },
    ...
    if (
      ...
      const finalMessages = [{ role: 'system', content: finalSystemPrompt }, ...formattedHistory];
      ...
      const { visible_response: finalVisibleResponse, assessment: finalAssessment } =
        llmService.parseAssessment(finalResponse.content);
  ...
  async _handlePolicyFailure(
    ...
    if (policyResult.reason_code === REASON_CODES.MISSING_PARAM && !policyResult.message) {
      ...
      const modelMessage =
        repromptResponse.content ||
        'I need some more information to help you. Could you please provide a few more details?';
  ...
  enforceServerPolicies(assessment, tools) {
    ...
    console.log(
      '[Policy] enforceServerPolicies called for tool:',
      toolName,
      'params:',
      Object.keys(assessment.tool_params || {})
    );
    ...
    console.log(
      '[Policy] Tool found, checking schema. Has parameters_schema:',
    ...
    console.log(
      '[Policy] Validation check - Required:',
      required,
      'Provided:',
      provided,
      'Missing:',
    ...
    if (effective_confidence < original_confidence) {
      console.log(
        `[Policy] Confidence floor applied: ${original_confidence} -> ${effective_confidence} for tool ${toolName} (max: ${policy.maxConfidence})`
      );
    }
    ...
    return {
      allowed: true,
      reason_code:
        effective_confidence < original_confidence ? REASON_CODES.CONFIDENCE_FLOOR_APPLIED : null,
      updated_assessment,
    };
  }
  ...
  async runCritique(
    ...
    try {
      ...
      const critiqueResponse = await llmService.chat([{ role: 'user', content: critiquePrompt }], {
      ...
      console.error(
        `[Critique] Failed (attempt ${retryCount + 1}/${ADAPTIVE_REASONING.CRITIQUE_MAX_RETRIES + 1}):`,
        error.message
      );
      ...
      if (retryCount < ADAPTIVE_REASONING.CRITIQUE_MAX_RETRIES) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, ADAPTIVE_REASONING.CRITIQUE_RETRY_DELAY)
        );
        return await this.runCritique(
          userMessage,
          assessment,
          tools,
          conversationHistory,
    ...
    switch (decision) {
          ...
          return await this.actOnCritiqueDecision(
            {
              ...critiqueResult,
              decision: 'ESCALATE',
              reasoning: 'AI could not correct its understanding after retry',
            },
            assessment,
            visible_response,
            conversationId,
      ...
      case 'ASK_USER': {
        ...
        await Message.create(
          ...
          'assistant',
          userMessage.content,
          userMessage.tokens || 0
        );

        return {
          response: userMessage.content,
          tool_executed: false,
          reason_code: assessment.is_destructive
            ? REASON_CODES.AWAITING_CONFIRMATION
            : REASON_CODES.ASK_USER,
        };
      }
      ...
      case 'ESCALATE': {
        ...
        await escalationService.escalate(conversationId, 'ai_stuck');
        ...
        await Message.create(
          conversationId,
          'assistant',
          escalationMessage.content,
          escalationMessage.tokens || 0
        );
      ...
      default:
        return await this.actOnCritiqueDecision(
          {
            decision: 'ESCALATE',
            reasoning: 'Unknown critique decision',
            ask_reason: 'error',
            missing_info: [],
          },
          assessment,
   ...
   */
  async regenerateResponseWithCritiqueFeedback(
    critiqueResult,
    assessment,
    client,
    formattedHistory
    ...
    const messages = [{ role: 'system', content: modifiedSystemPrompt }, ...conversationMessages];
  ...
  async retryReasoning(
    ...
    await Message.createDebug(conversationId, 'assistant', critiqueDisplay, 'critique', {
      metadata: newCritiqueResult,
      reasonCode: 'RETRY_CRITIQUE',
    });
