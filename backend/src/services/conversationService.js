...
class ConversationService {
    ...
    async processMessage(client, sessionId, userMessage, options = {}) {
        ...
        try {
            ...
            return await this._processStandardMode(conversation, client, sessionId, userMessage, isNewConversation, options);
     ...
     * @private
     */
    async _processStandardMode(conversation, client, sessionId, userMessage, isNewConversation, options) {
        // Get enabled tools and prepare context
        const clientTools = await toolManager.getClientTools(client.id);
        const messages = await this._prepareMessagesForLLM(sessionId, client, clientTools, conversation, userMessage, options);
